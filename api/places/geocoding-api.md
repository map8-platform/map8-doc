...
- **Example** : `地址定位` (帶入 `address` 參數)
    ...
    - 而由於精確的定位結果到門牌，乃出自於 Map8 台灣圖霸電子地圖 API 平台最具權威的門牌地址資料庫，因此，`authoritative` 顯示為 `true`
...
- **Example** : `地址定位`，模糊搜尋的例子 (`address=太平村下坑6號`)
    ...
    - 儘管如此，Map8 台灣圖霸電子地圖 API 平台仍盡可能查詢到了有該地址的景點登錄資料。因此，可以看到 `"level": "fuzzy"` 表示定位結果為搜尋而得，且信心指數 `likelihood` 超過 80%
...
- **Example** : `地址定位`，內插的例子 (`address=新北市板橋區金門街215巷76之3號`)
    ...
    ```
    - 可以看到 `"authoritative": "false"`，表示定位結果之 `formatted_address` 並不存在於 Map8 台灣圖霸電子地圖 API 平台最具權威的門牌地址資料庫內
